...
export class CodexTool extends Tool {
  ...
  private getFileContent(filePath: string): Pick<Session, 'content' | 'jsonContent' | 'summary' | 'path'> {
    ...
    const messageTypes = ['user_message', 'token_count', 'turn_context', 'task_started', 'task_complete'];
    const summaryContent = jsonContent.filter((c) => messageTypes.some(t => t === c.type || (c.type === 'event_msg' && t === c.payload.type)));
